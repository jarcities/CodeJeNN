...
""",
        'leakyRelu': """
auto leakyRelu = [](auto& output, auto input, auto alpha = 0.01) noexcept {
...
auto selu = [](auto& output, auto input, auto alpha = SELU_ALPHA<double>) noexcept {
    using Scalar = decltype(input);
    output = SELU_LAMBDA<Scalar> * (input > 0 ? input : alpha * (std::exp(input) - 1));
};
""",
        'swish': """
auto swish = [](auto& output, auto input, auto alpha = 1.0) noexcept {
    output = input / (1 + std::exp(-alpha * input));
};
""",
        'prelu': """
auto prelu = [](auto& output, auto input, auto alpha) noexcept {
    output = input > 0 ? input : alpha * input;
};
""",
        'silu': """
...
"""
    ...
    unique_activations = {('tanhCustom' if act == 'tanh' else act) for act in unique_activations if act is not None}

    for act in unique_activations:
        if act in lambda_defs:
            cpp_code += lambda_defs[act]
