...
//     forwardPass<Scalar, 8>(layer_3_output.data(), layer_2_output.data(), weights_3.data(), biases_3.data(), 3, &linear<Scalar>, 0.0);

//     std::array<Scalar, 8> layer_4_output;
//     forwardPass<Scalar, 8>(layer_4_output.data(), layer_3_output.data(), weights_4.data(), biases_4.data(), 8, &silu<Scalar>, 0.0);
...
//     forwardPass<Scalar, 8>(layer_6_output.data(), layer_5_output.data(), weights_6.data(), biases_6.data(), 8, &tanhCustom<Scalar>, 0.0);
...
//     forwardPass<Scalar, 8>(layer_8_output.data(), layer_7_output.data(), weights_8.data(), biases_8.data(), 8, &linear<Scalar>, 0.0);
...
//     forwardPass<Scalar, 8>(layer_12_output.data(), layer_11_output.data(), weights_12.data(), biases_12.data(), 8, &linear<Scalar>, 0.0);
...
//     forwardPass<Scalar, 10>(layer_14_output.data(), layer_13_output.data(), weights_14.data(), biases_14.data(), 8, &linear<Scalar>, 0.0);
...
// Updated activationFunction type to operate on single elements
template<typename Scalar>
using activationFunction = void(*)(Scalar&, Scalar, Scalar) noexcept;

// - -

// Per-element activation functions
template<typename Scalar>
inline void relu(Scalar& output, const Scalar input, [[maybe_unused]] Scalar alpha = 0.0) noexcept {
    // Use std::max for clarity and potential optimization hints.
    output = std::max(input, Scalar(0));
}

template<typename Scalar>
inline void linear(Scalar& output, const Scalar input, [[maybe_unused]] Scalar alpha = 0.0) noexcept {
    output = input;
}

template<typename Scalar>
inline void silu(Scalar& output, const Scalar input, [[maybe_unused]] Scalar alpha = 1.0) noexcept {
    const Scalar val = (Scalar(1) / (Scalar(1) + std::exp(-input)));
    output = input * val;
}

template<typename Scalar>
inline void tanhCustom(Scalar& output, const Scalar input, [[maybe_unused]] Scalar alpha = 0.0) noexcept {
    output = std::tanh(input);
}

template<typename Scalar>
inline void sigmoid(Scalar& output, const Scalar input, [[maybe_unused]] Scalar alpha = 0.0) noexcept {
    output = Scalar(1) / (Scalar(1) + std::exp(-input));
}

template<typename Scalar>
inline void elu(Scalar& output, const Scalar input, const Scalar alpha) noexcept {
    output = (input > Scalar(0)) ? input : alpha * (std::exp(input) - Scalar(1));
}

template<typename Scalar, int size>
inline void batchNormalization(Scalar* outputs, const Scalar* inputs, const Scalar* gamma, const Scalar* beta, const Scalar* mean, const Scalar* variance, const Scalar epsilon) noexcept {
    for (int i = 0; i < size; ++i) {
        outputs[i] = gamma[i] * ((inputs[i] - mean[i]) / std::sqrt(variance[i] + epsilon)) + beta[i];
    }
}
...
inline void layerNormalization(Scalar* outputs, const Scalar* inputs, const Scalar* gamma, const Scalar* beta, Scalar epsilon) noexcept {
    ...
    mean /= static_cast<Scalar>(size);
    for (int i = 0; i < size; ++i) {
        const Scalar diff = (inputs[i] - mean);
        variance += diff * diff;
    }
    variance /= static_cast<Scalar>(size);
    const Scalar inv = Scalar(1) / std::sqrt(variance + epsilon);
    for (int i = 0; i < size; ++i) {
        outputs[i] = gamma[i] * ((inputs[i] - mean) * inv) + beta[i];
    }
}

// Modified forwardPass with a regular for loop for the dot product
template<typename Scalar, int output_size>
inline void forwardPass(Scalar* outputs, const Scalar* inputs, const Scalar* weights, const Scalar* biases, int input_size, activationFunction<Scalar> activation_function, Scalar alpha) noexcept {
    for (int i = 0; i < output_size; ++i) {
        Scalar sum = Scalar(0);
...
inline auto newForLoop(const std::array<Scalar, 3>& initial_input) { 
    if (initial_input.size() != 3) { throw std::invalid_argument("Invalid input size. Expected size: 3"); } 
    ...
    std::array<Scalar, 8> gamma_5 = {1.007159472e+00, 9.102494121e-01, 9.754046202e-01, 1.009639382e+00, 9.939744473e-01, 1.053939223e+00, 1.005302906e+00, 1.049999237e+00};
    std::array<Scalar, 8> beta_5 = {-6.327476352e-02, 1.301053464e-01, -8.200552315e-02, 3.901715577e-02, 1.436686069e-01, 1.086948216e-01, 9.198349435e-04, 1.006997824e-01};
    const Scalar epsilon_5 = 1.000000000e-03;
    ...
    std::array<Scalar, 8> mean_7 = {9.823068976e-01, 8.114451170e-01, 7.645511031e-01, -6.787924170e-01, -8.765227199e-01, 9.079492688e-01, 6.484096050e-01, 9.349939227e-01};
    std::array<Scalar, 8> variance_7 = {1.048183767e-04, 9.622686543e-03, 1.916389167e-02, 8.062008768e-02, 2.308205934e-03, 1.166001894e-02, 6.997291744e-02, 2.667167410e-02};
    const Scalar epsilon_7 = 1.000000000e-03;
    ...
    std::array<Scalar, 8> gamma_13 = {7.639720440e-01, 8.527194262e-01, 6.928566098e-01, 8.144926429e-01, 9.072342515e-01, 9.955703616e-01, 8.884121776e-01, 8.234179020e-01};
    std::array<Scalar, 8> beta_13 = {2.525599673e-02, -8.494965732e-02, 4.972323775e-02, 6.251193583e-02, 1.001451388e-01, -1.113965511e-01, 8.469219506e-02, -4.730454832e-02};
    const Scalar epsilon_13 = 1.000000000e-03;
    ...
    std::array<Scalar, 8> layer_9_output;
    for (int i = 0; i < 8; ++i) {
        sigmoid<Scalar>(layer_9_output[i], layer_8_output[i], 0.0);
    }

    std::array<Scalar, 8> layer_10_output;
    for (int i = 0; i < 8; ++i) {
        linear<Scalar>(layer_10_output[i], layer_9_output[i], 0.0);
    }

    std::array<Scalar, 8> layer_11_output;
    for (int i = 0; i < 8; ++i) {
        elu<Scalar>(layer_11_output[i], layer_10_output[i], 1.0);
    }
