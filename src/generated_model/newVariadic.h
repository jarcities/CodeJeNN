...
// Per-element addBias and dotProduct (unchanged)
template<typename Scalar>
...
inline void forwardPassImpl(Scalar* outputs,
    ...
    ( (
        ...
        [&outputs, &inputs, &weights, &biases, input_size, &activation_function, alpha]() {
            Scalar sum = 0;
            for(int j = 0; j < input_size; ++j){
                sum += inputs[j] * weights[j * output_size + Is];
            }
            // Add bias
            sum += biases[Is];
            // Apply activation function
            activation_function(outputs[Is], sum, alpha);
        }()
    ), ...);
}
