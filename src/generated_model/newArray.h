...
template<typename Scalar>
void relu(Scalar& o,Scalar i,Scalar a=0.0)noexcept{o=i>0?i:0;}
template<typename Scalar>
void linear(Scalar& o,Scalar i,Scalar a=0.0)noexcept{o=i;}
template<typename Scalar>
void silu(Scalar& o,Scalar i,Scalar a=1.0)noexcept{Scalar s=1/(1+std::exp(-i));o=i*s;}
template<typename Scalar>
void tanhCustom(Scalar& o,Scalar i,Scalar a=0.0)noexcept{o=std::tanh(i);}
template<typename Scalar>
void sigmoid(Scalar& o,Scalar i,Scalar a=0.0)noexcept{o=1/(1+std::exp(-i));}
template<typename Scalar>
void elu(Scalar& o,Scalar i,Scalar a)noexcept{o=i>0?i:a*(std::exp(i)-1);}
template<typename Scalar>
void addBias(Scalar& o,Scalar b)noexcept{o+=b;}
template<typename Scalar>
void dotProduct(Scalar& s,Scalar in,Scalar w)noexcept{s+=in*w;}
...
inline void forwardPassImpl(Scalar* o,const Scalar* in,const std::array<std::array<Scalar,output_size>,input_size>& w,const Scalar* b,activationFunction<Scalar> f,Scalar a,std::index_sequence<Is...>)noexcept{
    (([&]{Scalar S=0;for(int j=0;j<input_size;++j)S+=in[j]*w[j][Is];S+=b[Is];f(o[Is],S,a);}()),...);
}
