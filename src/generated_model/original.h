...
auto tanhCustom = [](auto& output, auto input, auto alpha = 0.0) noexcept {
    output = std::tanh(input);
};

auto sigmoid = [](auto& output, auto input, auto alpha = 0.0) noexcept {
    output = 1 / (1 + std::exp(-input));
};

auto relu = [](auto& output, auto input, auto alpha = 0.0) noexcept {
    output = input > 0 ? input : 0;
};

auto silu = [](auto& output, auto input, auto alpha = 1.0) noexcept {
    auto sigmoid = 1 / (1 + std::exp(-input));
    output = input * sigmoid;
};

auto linear = [](auto& output, auto input, auto alpha = 0.0) noexcept {
    output = input;
};

auto elu = [](auto& output, auto input, auto alpha = 1.0) noexcept {
...
auto original(const std::array<Scalar, 3>& initial_input) { 
    ...
    elu(layer_11_output[0], layer_10_output[0], 0.0);
    elu(layer_11_output[1], layer_10_output[1], 0.0);
    elu(layer_11_output[2], layer_10_output[2], 0.0);
    elu(layer_11_output[3], layer_10_output[3], 0.0);
    elu(layer_11_output[4], layer_10_output[4], 0.0);
    elu(layer_11_output[5], layer_10_output[5], 0.0);
    elu(layer_11_output[6], layer_10_output[6], 0.0);
    elu(layer_11_output[7], layer_10_output[7], 0.0);
