...
INPUT_DIM = 100
OUTPUT_DIM = 10
input = np.random.rand(NUM_SAMPLES, INPUT_DIM, 1, 1)
...
#define model
inputs = layers.Input(shape=(INPUT_DIM, 1, 1))
#layer 1
x = layers.Conv2D(filters=32, kernel_size=(5, 1), activation='sigmoid', padding='same')(inputs)
x = layers.BatchNormalization()(x)
x = layers.AveragePooling2D(pool_size=(2, 1))(x)
#layer 2
x = layers.DepthwiseConv2D(kernel_size=(3, 1), activation='relu', padding='same')(x)
x = layers.BatchNormalization()(x)
#layer 3
x = layers.Conv2D(filters=64, kernel_size=(5, 1), padding='same')(x)
x = layers.Activation('silu')(x)
x = layers.BatchNormalization()(x)
x = layers.MaxPooling2D(pool_size=(2, 1))(x)
#layer 4
x = layers.SeparableConv2D(filters=96, kernel_size=(3, 1), activation='swish', padding='same')(x)
x = layers.BatchNormalization()(x)
#layer 5
x = layers.Conv2D(filters=128, kernel_size=(3, 1), activation='tanh', padding='same')(x)
x = layers.BatchNormalization()(x)
x = layers.MaxPooling2D(pool_size=(2, 1))(x)
#layer 6
x = layers.Conv2DTranspose(filters=64, kernel_size=(3, 1), strides=(2, 1), activation='softmax', padding='same')(x)
#x = layers.Conv2DTranspose(filters=64, kernel_size=(3, 1), strides=(2, 1), padding='same')(x)
